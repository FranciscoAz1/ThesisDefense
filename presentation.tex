...
\begin{frame}{Motivation: Enterprise Information Pain}
\begin{itemize}
  \item Public sector platforms: rich processes, poor retrieval
  \item Users lose time locating updated documents
  \item Decisions risk using superseded info
  \item Manual checking of dependencies = high cognitive load
\end{itemize}
\vspace{0.4em}
\textbf{Goal:} Reliable answers over evolving document ecosystems.
\end{frame}
...
\begin{itemize}
  \item Jan: Approve Vendor X (A)
  \item Mar: Suspend Vendor X (B)
  \item Apr Query: ``Can we contract?'' % chktex 38 allow
\end{itemize}
...
\textbf{Secondary goal:} Scalable, optimized storage solution. Efficient document processing (if required). Good explainable query accuracy.
\vspace{0.5em}
\end{frame}
...
\begin{enumerate}
  ...
  \item Agent traversal (multi-hop, reasoning)
