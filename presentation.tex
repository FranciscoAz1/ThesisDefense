...
\begin{figure}
        \centering
        \includegraphics[width=1\linewidth]{Images/edockling documents.png}
...
\begin{enumerate}
  \item \textbf{Instructions:} Instructed to be a document assistant connected to a Weaviate database; answers are concise and grounded with sources.
  \item \textbf{Tools} Tools via an MCP server that translate agent requests (natural language) to valid GraphQL over the schema.
\end{enumerate}
\end{frame}
...
\begin{frame}{Comparison to Prior Approaches}
    My solution is a Multi-Hop Agentic Retrieval: semi-structured enterprise information in ruled workflow configurations (e.g., Edoclink)
\begin{itemize}
...
\begin{itemize}
  ...
  \item More MCP for other applications (as LLMs can understand us and computer's too)
\end{itemize}
\end{frame}
